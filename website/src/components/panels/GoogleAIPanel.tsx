...
export const GoogleAIPanel: React.FC = () => {
  const { user } = useAuth();
  ...
  const handleSave = async () => {
    ...
    try {
      await updateDoc(doc(db, 'sasha_settings', user.uid), {
        'integrations.google.apiKey': apiKey,
        lastUpdated: new Date().toISOString()
      });
      setStatus('success');
    } catch (error) {
      console.error('Error saving Google AI settings:', error);
      setStatus('error');
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold">Google AI Integration</h3>
